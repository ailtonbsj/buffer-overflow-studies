...
; Set video mode. Mode 13h is VGA (1 byte per pixel with the actual color
; stored in a palette), 320x200 total size. When restarting, this also clears the screen.
    mov ax, 0x0013
    int 0x10
...
game_loop:
; We read the keyboard input from port 0x60. This also reads bytes from the mouse,
; so we need to only handle [up (0x48), left (0x4b), right (0x4d), down (0x50)]
    in al, 0x60
    cmp al, 0x48
    jb kb_handle_end
    cmp al, 0x50
...
; At the end bx contains offset displacement (+1, -1, +320, -320) based on 
; pressed/released keypad key. I bet there are a few bytes
...
; Today's computers are however too fast, so we use int 15h 86h instead.
; This also shaves a few bytes.

; Note: you'll have to tweak cx+dx if you are running this on a virtual machine
; vs real hardware. Casual testing seems to show that virtual machines
